...
When a change from another instance of the document is receieved, the teletype-crdt document can be notified using the integrateOperations (see \autoref{lst:integrateOperations}) function. This function returns a set of TextUpdates, which are changes to the text of the document to match the operations.
...
Upon receieving this information, the bootstrap server responds with a list of IP addresses and ports of all the other registered peers.
